...
export class ToolCallingAdapter {
   ...
   public bindTools(tools: ToolSpec[], options: { tool_choice?: 'auto' | 'none' | 'required' | { type: 'tool', name: string } } = { tool_choice: 'auto' }) {
      tools.forEach((t, idx) => {
         if (!t?.name || typeof t.name !== 'string') {
            t.name = `tool_${idx}`;
         }
         if (!t.schema || typeof t.schema !== 'object' || t.schema.type !== 'object') {
            t.schema = { type: 'object', properties: { input: { type: 'string' } }, required: ['input'] };
         }
      });
      ...
      const requireToolName = typeof options?.tool_choice === 'object' && options.tool_choice?.type === 'tool'
      ...
      return {
         invoke: async (input: { messages: BaseMessage[] } | BaseMessage[]) => {
            ...
            const lastUserMessage = nonSystem.find(m => m.type === 'human') || nonSystem[nonSystem.length - 1];
            const augmented = [
               new AIMessage({ content: systemInstruction, type: 'system' }),
               ...nonSystem,
            ];
            ...
            try {
               ...
               console.error('[ERROR] ToolCallingAdapter invoke error:', error.message);
               return new AIMessage(`Ошибка при обращении к модели: ${error.message}`);
            }
         },
      } as BaseInvokeModel;
   }
